...
<div class="container navbar-space">
    ...
    <?php
        class SearchRoom extends Connect {
            ...
            private function renderRow($row) {
                ...
                return "<tr>
                    ...
                    <td class='text-center'>
                        <button type='button' class='btn btn-sm btn-danger' onclick=\"openAuthModal('delete', '$roomId')\">
                            <i class='fa-solid fa-trash'></i>
                        </button>
                        <button type='button' class='btn btn-sm btn-primary ms-1' onclick=\"openAuthModal('edit', '$roomId')\">
            ...
            private function renderCard($row) {
                ...
                return "<div class='card shadow-sm mb-3'>
                    ...
                    <div class='card-body'>
                        ...
                        <button type='button' class='btn btn-sm btn-danger w-100 mt-2' onclick=\"openAuthModal('delete', '$roomId')\">
                            <i class='fa-solid fa-trash'></i>
                        </button>
                        <button type='button' class='btn btn-sm btn-primary mt-2' onclick=\"openAuthModal('edit', '$roomId')\">
                            <i class='fa-solid fa-pen-to-square'></i>
                        </button>
                    </div>
                </div>";
            ...
            // ✅ delete method
            public function deleteRoomById($roomId, $hotel_id) {
                $stmt = $this->db_handle->prepare("DELETE FROM room WHERE room_id=? AND hotel_id=?");
                $stmt->bind_param("ii", $roomId, $hotel_id);
                if ($stmt->execute()) {
                    echo "<script>alert('Room Deleted Successfully'); window.location.href='searchroom.php';</script>";
                    exit;
                } else {
                    echo "<div class='alert alert-danger text-center mt-3'>Error deleting room.</div>";
                }
                $stmt->close();
            }
        ...
        // ✅ Admin authentication for delete/edit (hardcoded same as searchguest1.php)
        if (isset($_POST['action']) && isset($_POST['admin_id']) && isset($_POST['admin_pass'])) {
            ...
            if ($_POST['admin_id'] === $adminEmail && $_POST['admin_pass'] === $adminPass) {
                if ($_POST['action'] == "delete") {
                    $search->deleteRoomById($_POST['room_id'], $hotel_id);
                } elseif ($_POST['action'] == "edit") {
                    echo "<script>window.location.href='room.php?room_id=" . urlencode($_POST['room_id']) . "';</script>";
                    exit;
                }
            } else {
...
<script>
function openAuthModal(action, roomId) {
